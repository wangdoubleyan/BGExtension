//
//  BGNavigationBar_Extension.swift
//  BGExtension
//
...
extension UINavigationBar:BGAwakeProtocol
{
    ...
    fileprivate func bg_setBackgroundImage(image:UIImage)
    {
        ...
        if (backgroundImageView == nil)
        {
            ...
            backgroundImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: BGNavigationBar.navBarBottom()))
    ...
    fileprivate func bg_setBackgroundColor(color:UIColor)
    {
        ...
        if (backgroundView == nil)
        {
            // add a image(nil color) to _UIBarBackground make it clear
            setBackgroundImage(UIImage(), for: .default)
            backgroundView = UIView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: BGNavigationBar.navBarBottom()))
    ...
    fileprivate func bg_setBackgroundAlpha(alpha:CGFloat)
    {
    ...
    func bg_setBarButtonItemsAlpha(alpha:CGFloat, hasSystemBackIndicator:Bool)
    {
    ...
    /// 设置导航栏在垂直方向上平移多少距离
    func bg_setTranslationY(translationY:CGFloat)
    {
        transform = CGAffineTransform.init(translationX: 0, y: translationY)
    }
    
    func bg_getTranslationY() -> CGFloat
    {
        return transform.ty
    }
    ...
    public static func bgAwake()
    {
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                let str = ("bg_" + selector.description)
    ...
    // MARK: swizzling pop
    //==========================================================================
    @objc func bg_setTitleTextAttributes(_ newTitleTextAttributes:[String : Any]?)
    {
        ...
        guard let originTitleTextAttributes = titleTextAttributes else {
            bg_setTitleTextAttributes(attributes)
            return
        }
...
//==========================================================================
extension UINavigationController: BGFatherAwakeProtocol
{
    override open var preferredStatusBarStyle: UIStatusBarStyle {
        return topViewController?.statusBarStyle ?? BGNavigationBar.defaultStatusBarStyle
    }
    
    fileprivate func setNeedsNavigationBarUpdate(backgroundImage: UIImage)
    {
        navigationBar.bg_setBackgroundImage(image: backgroundImage)
    }
    
    fileprivate func setNeedsNavigationBarUpdate(barTintColor: UIColor)
    {
        navigationBar.bg_setBackgroundColor(color: barTintColor)
    }
    
    fileprivate func setNeedsNavigationBarUpdate(barBackgroundAlpha: CGFloat)
    {
        navigationBar.bg_setBackgroundAlpha(alpha: barBackgroundAlpha)
    }
    ...
    fileprivate func updateNavigationBar(fromVC: UIViewController?, toVC: UIViewController?, progress: CGFloat)
    {
        ...
        let fromBarTintColor = fromVC?.navBarBarTintColor ?? BGNavigationBar.defaultNavBarBarTintColor
        let toBarTintColor   = toVC?.navBarBarTintColor ?? BGNavigationBar.defaultNavBarBarTintColor
        let newBarTintColor  = BGNavigationBar.middleColor(fromColor: fromBarTintColor, toColor: toBarTintColor, percent: progress)
        setNeedsNavigationBarUpdate(barTintColor: newBarTintColor)
        
        // change navBarTintColor
        let fromTintColor = fromVC?.navBarTintColor ?? BGNavigationBar.defaultNavBarTintColor
        let toTintColor = toVC?.navBarTintColor ?? BGNavigationBar.defaultNavBarTintColor
        let newTintColor = BGNavigationBar.middleColor(fromColor: fromTintColor, toColor: toTintColor, percent: progress)
        setNeedsNavigationBarUpdate(tintColor: newTintColor)
        
        // change navBarTitleColor
        //        let fromTitleColor = fromVC?.navBarTitleColor ?? BGNavigationBar.defaultNavBarTitleColor
        //        let toTitleColor = toVC?.navBarTitleColor ?? BGNavigationBar.defaultNavBarTitleColor
        //        let newTitleColor = BGNavigationBar.middleColor(fromColor: fromTitleColor, toColor: toTitleColor, percent: progress)
        //        setNeedsNavigationBarUpdate(titleColor: newTitleColor)
        
        // change navBar _UIBarBackground alpha
        let fromBarBackgroundAlpha = fromVC?.navBarBackgroundAlpha ?? BGNavigationBar.defaultBackgroundAlpha
        let toBarBackgroundAlpha = toVC?.navBarBackgroundAlpha ?? BGNavigationBar.defaultBackgroundAlpha
        let newBarBackgroundAlpha = BGNavigationBar.middleAlpha(fromAlpha: fromBarBackgroundAlpha, toAlpha: toBarBackgroundAlpha, percent: progress)
    ...
    public static func fatherAwake()
    {
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                // _updateInteractiveTransition:  =>  bg_updateInteractiveTransition:
                let str = ("bg_" + selector.description).replacingOccurrences(of: "__", with: "_")
                if let originalMethod = class_getInstanceMethod(self, selector),
                    let swizzledMethod = class_getInstanceMethod(self, Selector(str)) {
                    method_exchangeImplementations(originalMethod, swizzledMethod)
                }
    ...
    @objc func bg_popToViewController(_ viewController: UIViewController, animated: Bool) -> [UIViewController]?
    {
        ...
        CATransaction.begin()
        let vcs = bg_popToViewController(viewController, animated: animated)
        CATransaction.commit()
        return vcs
    }
    
    // swizzling system method: popToRootViewControllerAnimated
    @objc func bg_popToRootViewControllerAnimated(_ animated: Bool) -> [UIViewController]?
    {
        var displayLink:CADisplayLink? = CADisplayLink(target: self, selector: #selector(popNeedDisplay))
        displayLink?.add(to: RunLoop.main, forMode: .commonModes)
        CATransaction.setCompletionBlock {
        ...
        CATransaction.setAnimationDuration(popProperties.popDuration)
        CATransaction.begin()
        let vcs = bg_popToRootViewControllerAnimated(animated)
        CATransaction.commit()
        return vcs;
    }
    ...
    @objc func bg_pushViewController(_ viewController: UIViewController, animated: Bool)
    {
        ...
        CATransaction.begin()
        bg_pushViewController(viewController, animated: animated)
        CATransaction.commit()
    }
...
extension UINavigationController: UINavigationBarDelegate
{
    ...
    private func dealInteractionChanges(_ context: UIViewControllerTransitionCoordinatorContext)
    {
        let animations: (UITransitionContextViewControllerKey) -> () = {
            let curColor = context.viewController(forKey: $0)?.navBarBarTintColor ?? BGNavigationBar.defaultNavBarBarTintColor
            let curAlpha = context.viewController(forKey: $0)?.navBarBackgroundAlpha ?? BGNavigationBar.defaultBackgroundAlpha
    ...
    @objc func bg_updateInteractiveTransition(_ percentComplete: CGFloat)
    {
        guard let topViewController = topViewController,
            let coordinator       = topViewController.transitionCoordinator else {
                bg_updateInteractiveTransition(percentComplete)
                return
        }
...
//=============================================================================
extension UIViewController: BGAwakeProtocol
{
    fileprivate struct AssociatedKeys
    {
        ...
        static var navBarBarTintColor: UIColor = BGNavigationBar.defaultNavBarBarTintColor
        static var navBarBackgroundAlpha:CGFloat = 1.0
        static var navBarTintColor: UIColor = BGNavigationBar.defaultNavBarTintColor
        static var navBarTitleColor: UIColor = BGNavigationBar.defaultNavBarTitleColor
    ...
    var navBarBackgroundImage: UIImage?
    {
        get {
            guard let bgImage = objc_getAssociatedObject(self, &AssociatedKeys.navBarBackgroundImage) as? UIImage else {
                return BGNavigationBar.defaultNavBarBackgroundImage
            }
            return bgImage
        }
        //        set {
        ...
        //                navBar.bg_setBackgroundImage(image: newValue!)
        //            }
        //            else {
    ...
    var navBarBarTintColor: UIColor {
        get {
            guard let barTintColor = objc_getAssociatedObject(self, &AssociatedKeys.navBarBarTintColor) as? UIColor else {
                return BGNavigationBar.defaultNavBarBarTintColor
            }
            return barTintColor
        }
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                //                let navBar = customNavBar as! UINavigationBar
                //                navBar.bg_setBackgroundColor(color: newValue)
            }
            else {
    ...
    var navBarBackgroundAlpha:CGFloat {
        ...
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                //                let navBar = customNavBar as! UINavigationBar
                //                navBar.bg_setBackgroundAlpha(alpha: newValue)
            }
            else {
                if canUpdateNavBarBarTintColorOrBackgroundAlpha == true {
    ...
    var navBarTintColor: UIColor {
        get {
            guard let tintColor = objc_getAssociatedObject(self, &AssociatedKeys.navBarTintColor) as? UIColor else {
                return BGNavigationBar.defaultNavBarTintColor
            }
            return tintColor
        }
        set {
    ...
    var navBarTitleColor: UIColor {
        get {
            guard let titleColor = objc_getAssociatedObject(self, &AssociatedKeys.navBarTitleColor) as? UIColor else {
                return BGNavigationBar.defaultNavBarTitleColor
            }
            return titleColor
        }
        set {
    ...
    var statusBarStyle: UIStatusBarStyle {
        get {
            guard let style = objc_getAssociatedObject(self, &AssociatedKeys.statusBarStyle) as? UIStatusBarStyle else {
                return BGNavigationBar.defaultStatusBarStyle
            }
            return style
        }
        set {
    ...
    var navBarShadowImageHidden:Bool {
        get {
            guard let isHidden = objc_getAssociatedObject(self, &AssociatedKeys.navBarShadowImageHidden) as? Bool else {
                return BGNavigationBar.defaultShadowImageHidden
            }
            return isHidden
        }
        set {
    ...
    private static let onceToken = UUID().uuidString
    @objc public static func bgAwake()
    {
        DispatchQueue.once(token: onceToken)
        {
            ...
            {
                let newSelectorStr = "bg_" + selector.description
                if let originalMethod = class_getInstanceMethod(self, selector),
    ...
    @objc func bg_viewWillAppear(_ animated: Bool)
    {
        ...
        bg_viewWillAppear(animated)
    }
    
    @objc func bg_viewWillDisappear(_ animated: Bool)
    {
        if canUpdateNavigationBar() == true {
            pushToNextVCFinished = true
        }
        bg_viewWillDisappear(animated)
    }
    
    @objc func bg_viewDidAppear(_ animated: Bool)
    {
    ...
    func canUpdateNavigationBar() -> Bool
    {
        ...
        let middleFrame = CGRect(x: 0, y: BGNavigationBar.navBarBottom(), width: BGNavigationBar.screenWidth(), height: BGNavigationBar.screenHeight()-BGNavigationBar.navBarBottom())
        let minFrame = CGRect(x: 0, y: BGNavigationBar.navBarBottom(), width: BGNavigationBar.screenWidth(), height: BGNavigationBar.screenHeight()-BGNavigationBar.navBarBottom()-BGNavigationBar.tabBarHeight())
        // 蝙蝠🦇
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 1. 定义 BGAwakeProtocol 协议
public protocol BGAwakeProtocol: class {
    static func bgAwake()
}
public protocol BGFatherAwakeProtocol: class
{   // 1.1 定义 BGFatherAwakeProtocol ()
...
class NothingToSeeHere
{
    static func harmlessFunction(){
        ...
        //            (types[index] as? BGAwakeProtocol.Type)?.BGAwake() //如果该类实现了SelfAware协议，那么调用 awake 方法
        //            (types[index] as? BGFatherAwakeProtocol.Type)?.fatherAwake()
        //        }
        //        types.deallocate(capacity: typeCount)
        UINavigationBar.bgAwake()
        UIViewController.bgAwake()
